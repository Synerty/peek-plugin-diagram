...
import {PrivateDiagramBranchLoaderServiceA} from "./PrivateDiagramBranchLoaderServiceA";
 ...
 */
@Injectable()
export class PrivateDiagramBranchLoaderService extends PrivateDiagramBranchLoaderServiceA {
    ...
    private _statusSubject = new Subject<PrivateDiagramBranchLoaderStatusTuple>();
    private _status = new PrivateDiagramBranchLoaderStatusTuple();
    ...
    statusObservable(): Observable<PrivateDiagramBranchLoaderStatusTuple> {
        return this._statusSubject;
    }

    status(): PrivateDiagramBranchLoaderStatusTuple {
        return this._status;
    }
    ...
    private processGridsFromServer(payloadEnvelope: PayloadEnvelope) {
        payloadEnvelope.decodePayload()
            ...
            .catch(e => `ERROR BranchLoader.processGridsFromServer: ${e}`);
    }
    ...
    private emitEncodedGridTuples(encodedGridTuples: EncodedGridTuple[]): void {
        ...
        for (let encodedGridTuple of encodedGridTuples) {
            let promise: any = Payload.fromEncodedPayload(encodedGridTuple.encodedGridTuple)
                ...
                .catch((err) => {
                    console.log(`BranchLoader.emitEncodedGridTuples decode error: ${err}`);
                });
        ...
        Promise.all(promises)
            ...
            .catch((err) => {
                console.log(`BranchLoader.emitEncodedGridTuples all error: ${err}`);
            });
